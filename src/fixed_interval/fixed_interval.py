...
def main(testing=True):
    ...
    while True:
        if task_helper.levers_output(left=True, right=True, testing=testing):
            ratio = task_helper.calc_func(params['ratio_function'], trial_num)
            interval = task_helper.calc_func(params['interval_function'], trial_num)

            try:
                duration, lever = task_helper.levers_input(timeout=params['TO_interval'], ratio=ratio, interval=interval, testing=testing)
                task_helper.dispense_pellet(num=params['reward_num'], testing=testing)
            except TimeoutError:
                duration = None
                print('Lever Timeout')

            #TODO: fix output row for ratio stuff
            output_row = {
                'response': trial_num,
                'duration': duration,
                'IRT': None,
